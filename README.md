...
* **Auto-Downloader & Configurator:** Missing Tosu? The script fetches the latest Windows release directly from GitHub, extracts it, and applies the optimal configuration automatically.
...
* **Dynamic Icon Support:** Drop your favorite `.ico` files into the `resources/icons/` folder, and the setup will let you choose your launcher icon dynamically.
...
> Make sure you are using the latest version of the script. The Tosu GitHub repository recently migrated. This launcher is updated to pull directly from `tosuapp/tosu`.

**Q: My antivirus / Windows Defender flagged the `.exe` file.**
> Because `setup.bat` compiles a C# executable that wraps and executes a PowerShell script silently in the background, heuristic scanners (like Windows Defender) might flag it as suspicious. This is a false positive. You can inspect the source code in `setup.bat` to see exactly how the C# wrapper is built. If issues persist, deleting the generated `OsuShadowLauncher.exe` and rebuilding it often helps.
